...
for item in items:
    ...
    if brand in VIDEOS.keys() and item['item_type'] == 'Full Episode':
        VIDEOS[brand].append({'name':item['title'], 'video':item['url'],'thumb':item['image_1'], 'genre':'Series'})
...
def get_url(**kwargs):
    ...
    return '{0}?{1}'.format(_url.encode('utf-8').decode('utf-8'), urlencode(kwargs))
...
def get_videos(category):
    ...
    :rtype: list
    """
    return VIDEOS[category.decode('utf-8')]
